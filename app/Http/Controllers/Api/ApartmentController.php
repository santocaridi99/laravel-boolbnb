    ...
    public function index(Request $request)
    {
        ...
        if ($filter) {
            ...
            if ($filter && $rooms && $beds) {
                $notFilteredApartments = Apartment::where("isVisible", "=", "1")->where("room_numbers", "=", "$rooms")->where("bed_numbers", "=", "$beds")->paginate(5);
            } elseif ($filter && $rooms) {
                $notFilteredApartments = Apartment::where("isVisible", "=", "1")->where("room_numbers", "=", "$rooms")->paginate(5);
                ...
                $notFilteredApartments = Apartment::where("isVisible", "=", "1")->where("bed_numbers", "=", "$beds")->paginate(5);
            } else {
                $notFilteredApartments = Apartment::where("isVisible", "=", "1")->paginate(5);
            }
            ...
            if ($rooms && $beds) {
                $apartments = Apartment::where("isVisible", "=", "1")->where("room_numbers", "=", "$rooms")->where("bed_numbers", "=", "$beds")->paginate(5);
            } elseif ($rooms) {
                $apartments = Apartment::where("isVisible", "=", "1")->where("room_numbers", "=", "$rooms")->paginate(5);
            } elseif ($picked) {
                $apartments = Apartment::whereHas('tags', function ($query) use ($picked) {
                    $query->where('tag_id', $picked)->where("isVisible", "=", "1");
                })->paginate(5);
            } elseif ($beds) {
                $apartments = Apartment::where("isVisible", "=", "1")->where("bed_numbers", "=", "$beds")->paginate(5);
            }else{
